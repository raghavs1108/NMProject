...
class SNMPAgent(object):
    ...
    def __init__(self, mibObjects):
        ...
        config.addSocketTransport(self._snmpEngine, udp.domainName,
                                  udp.UdpTransport().openServerMode(('', 163)))
...
class Worker(threading.Thread):
    ...
    def run(self):
        while True:
            time.sleep(3)
            print mib.getTestCount()+1
            self._mib.setTestCount(mib.getTestCount()+1)
            self._agent.sendTrap()

if __name__ == '__main__':
    mib = Mib()
    objects = [MibObject('ExampleMIB', 'testDescription', mib.getTestDescription),
               MibObject('ExampleMIB', 'testCount', mib.getTestCount)]
    agent = SNMPAgent(objects)
    agent.setTrapReceiver('192.168.0.104', 'traps')
